...
<form role="search" method="get" class="flex gap-3" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	...
	<input type="search" id="s" name="s" autocomplete="off" placeholder="<?php esc_attr_e( 'Search…', 'blog-pro' ); ?>" value="<?php echo get_search_query(); ?>" class="flex-1 px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all">
